...
/**
 * 入站：WebSocket 帧解码（RFC 6455）→ text 消息 String；处理 close / ping / pong 控制帧。
 *
 * <p>规则：
 * <ul>
 *   <li>客户端→服务器帧必须掩码（RFC 6455 §5.1），否则 close(1002)</li>
 *   <li>仅支持单帧消息（FIN=1）；分片帧（FIN=0）拒绝 close(1003)</li>
 *   <li>单帧上限 64KB（协议 §1 消息大小上限），超出 close(1009)</li>
 *   <li>text payload 必须是合法 UTF-8，否则 close(1007)</li>
 *   <li>close 帧回显同码并关闭；ping 帧回 pong</li>
 * </ul>
 *
 * <p>线程模型：本类仅在 Netty eventLoop 线程运行，不触碰游戏对象。
 */
public class WsFrameDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
        ...
        switch (opcode) {
            case OP_TEXT:
                ...
                break;
            case OP_CLOSE:
                // 回显对端 close 码（默认 1000）后关闭
                int code = 1000;
                if (payload.length >= 2) {
                    code = ((payload[0] & 0xFF) << 8) | (payload[1] & 0xFF);
                }
                ctx.writeAndFlush(WsFrameEncoder.closeFrame(ctx.alloc(), code));
                ctx.close();
                break;
            case OP_PING:
                ctx.writeAndFlush(WsFrameEncoder.pongFrame(ctx.alloc(), payload));
                break;
            case OP_PONG:
                break; // 忽略
    ...
    private static void fail(ChannelHandlerContext ctx, int closeCode) {
        ctx.writeAndFlush(WsFrameEncoder.closeFrame(ctx.alloc(), closeCode));
        ctx.close();
    }
}
