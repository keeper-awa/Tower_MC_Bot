...
class Brain:
    ...
    def _build_messages(self, context: str, attach_plan_state: bool, light: bool = False) -> list:
        ...
        system += (
            ...
            "为后续步骤备料时只合成所需数量（craft_chain 传 count），避免把材料全部用完；"
            "合成（木板/木棍/工具/箱子/床等）一律用 craft_chain 技能（传目标物品 id，如 "
            "minecraft:stone_pickaxe；自动处理缺料收集/个人合成格/工作台）；命令方块/基岩等"
            "不可获取物品无法合成；craft_items 仅作底层备用。\n"
